...
For why functions are self-contained and how `spawn` works, see [Execution Model](relang-execution-model.md).
...
See [Execution Model — Functions](relang-execution-model.md#3-functions) for why this restriction exists.
...
**Execution modes (functions only):**
- `f()` — inline call, returns `T`
- `spawn f()` — parallel call, returns `*T`

See [Execution Model — Calling Modes](relang-execution-model.md#33-calling-modes) for details.
...
### 7.5 Why Unit Instead of none or Void?

`Unit` is the type for functions with no meaningful return value.

**Why not `none`?**
- `none` means "absence of a value" (used in optionals: `Int?`)
...
### 7.6 Why No Function Types?

See [Execution Model](relang-execution-model.md#32-self-contained-no-scope-capture) for the full rationale. In summary:
...
- [Execution Model](relang-execution-model.md) — Durable execution, spawn, self-contained rationale
- [ReLang v0.1 Canonique](relang-spec-v0.1-canonique.md) — Awaitables and `await` core semantics
