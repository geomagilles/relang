...
Failures compare structurally (same envelope fields and nested causes):

```relang
let r1 = await http.get("https://service-a.example")
let r2 = await http.get("https://service-a.example")

match (r1 & r2) {
    (f1: Failure & f2: Failure) -> {
        let same = (f1 == f2)    // structural equality over Failure envelope
        log("same failure payload: ${same}")
    }
    _ -> unit
}
```
