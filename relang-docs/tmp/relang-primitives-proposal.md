...
**Note**: `and`/`or` on booleans are **logical operators**, distinct from the **coordination operators** that work on awaitables. The compiler disambiguates based on operand types:

```relang
// Boolean logic (both operands are Bool)
let result: Bool = isValid and isActive       // logical AND, short-circuit

// Coordination (both operands are awaitables)
let combined: *(A & B) = task1 and task2      // coordination, parallel execution
```

**Type-based disambiguation**:
- If both operands are `Bool` → logical operator (short-circuit, returns `Bool`)
- If both operands are `*T` → coordination operator (parallel, returns `*(A & B)` or `*(A | B)`)
- Mixed operand types → compile error
...
**Note**: Always use `now()` in workflow code, not system time. This ensures deterministic replay.
...
`now()` returns deterministic time that's consistent across replays. System time would cause non-determinism on resume.
