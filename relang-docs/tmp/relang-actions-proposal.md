...
2. **Actions are memoized**: Replay returns same result
...
task.errors()       // [Failure]? — failure if failed
...
```relang
let task = http.get(url, timeout: 30s)
// Or
let task = timeout(http.get(url), 30s)
```
...
let task = retry(http.get(url), policy)
```
...
```relang
let result: HttpResponse | Failure = http.get(url)```
...
```relang
let resp = (await http.get(url))!   // raises on failure
```
...
On replay, completed actions return memoized results:

```relang
fn example(): Result {
    let a = (await http.get(url1))!   // First run: executes
                                       // Replay: returns memoized
...
```relang
match await task {
    r: Response -> // completed before cancel
    f: Failure -> match f.error {
