...
Resolution is explicit via the `await` keyword.
...
| `or` | any succeeds | `Failure` (with `AggregateFailure` payload containing all failures) |
...
A **tuple type** combines multiple values.
...
A **union type** represents one of several alternatives.
...
```relang
let result: T | Failure = await t 
```

## 6.1. The `await` Keyword
...
The `await` keyword unwraps an awaitable, yielding either its success value or a `Failure`.

### Precedence

`await` has the **lowest precedence** of all operators. This means:

```relang
await t1 or t2        // parsed as: await (t1 or t2)
await t1 and t2       // parsed as: await (t1 and t2)
await http.get(url)   // parsed as: await (http.get(url))
```

Use parentheses when needed for clarity:

```relang
(await t1) or t2      // await t1 first, then race with t2
```

### Failure Propagation with `!`

The `await` expression can be followed by `!` to propagate failures:
...
**Grammar rule**: A trailing `!` after an `await` expression applies to the await result, not the inner expression.

```relang
// Common pattern: await and unwrap in one expression
let user = await db.getUser(id)!
let resp = await http.get(url)!
let (a & b) = await (t1 and t2)!
```
...
**Note**: When all branches fail, the returned `Failure` contains an `AggregateFailure` payload with all individual failures. See [Failure Aggregation](#failure-aggregation-or) for details.
...
match r {
  ...
  f: Failure -> print("All failed: ${f.error}")  // f.error is AggregateFailure
}
```
...
match r {
  (user & orders)     -> ...
  cached: CachedData  -> ...
  f: Failure          -> ...  // May contain AggregateFailure if all branches failed
}
```
...
- `await and([*S]) : [S] | Failure`
- `await or([*S])  : S | Failure` (on total failure, `Failure.error` is `AggregateFailure`)

Notes:

- List coordination yields lists, not tuples.
...
- `ErrorType` is a value from a shared, serializable error space (structs, unions, lists, primitives).
...
If a failure originates from a primitive awaitable, `awaitableId` refers to that awaitable’s `id`.

Failures produced by coordination (`and`, `or`) may have no single origin and may leave `awaitableId` unset.
