...
- **Explicit effects** — Async operations return `*T` awaitable types (see [Awaitables](relang-awaitables.md))
...
Every `await` creates a **checkpoint** — a snapshot of execution state.
...
The `await` keyword resolves an asynchronous operation and persists the result. If execution fails after checkpoint 2, it resumes with `a` and `b` already available — `step1()` and `step2()` are not called again.
...
For detailed syntax, see [Functions](relang-functions.md).
...
A ReLang file can contain:
- **Functions (`fn`)**: Reusable units of work
- **Top-level code**: The file's entry point (durable)

All functions must be defined at **file level** — nested functions are not allowed.

```relang
// orders.rl

// Function — can be called inline or spawned
fn processOrder(orderId: String): OrderResult {
...
// Top-level code — runs when file executed directly
let result = await spawn processOrder(args[0])!
print(result)
```
...
```bash
relang orders.rl "order-123"
```

- Executes top-level code as durable execution
- File path serves as execution identity
...
// Spawn function from imported file
let result = await spawn orders.processOrder("order-123")!

// Or call inline
let result = orders.processOrder("order-123")
```

- All functions are importable
- Caller decides whether to spawn or call inline
- Top-level code does NOT run on import

### 8.4 Top-Level Arguments
...
fn retriable(): Result {
    // execution-level retry on failure
}
```
...
| Aspect | Behavior |
|--------|----------|
| **Checkpoints** | Created at each `await` |
...
- [Awaitables](relang-awaitables.md) — Awaitable types, coordination, failure handling
- [Functions](relang-functions.md) — Detailed function syntax and semantics
